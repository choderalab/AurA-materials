...
for state in state_order:
    ...
    phi_separate = np.load('../ASP_phi_psi_contour/phi_separate_%s.npy'%state)
    psi_separate = np.load('../ASP_phi_psi_contour/psi_separate_%s.npy'%state)
    ...
    plt.plot(phi.flatten(), psi.flatten(),'.',color = colorp[i],alpha=0.15,ms=2)
...
plt.savefig('phi_psi_scatter_all-in-one.png',bbox_inches='tight',dpi=300)
